...
void process_json(radar_omnipresense::radar_data *data, std::vector<std::string> msgs, std::string serialPort)
{
    for(int i = 0; i < msgs.size(); i++)
    {
        ...
        const cJSON *range = cJSON_GetObjectItemCaseSensitive(json, "range");
        ...
        if (speed != NULL)
        {
            ...
            //accesses the value for range and assigns it to info.range
            // deal with it whether it's a value (like pre 1.2) or string (1.2.0)
            if (range->valuestring == NULL)
                data->range = range->valuedouble;
            else
                data->range = atof(range->valuestring);

        }
        else 
        {
            ROS_INFO("Unsupported message type received");
        }
        free(json);
    }
}
    ...
    while (ros::ok())
    {
        if (connection.isConnected())
        {
            if (! is_initialized)
            {
                ...
                //string of format {"speed":#.##,"time":###.###}.     
                ROS_INFO("setting radar settings");
                connection.clearBuffer();
                connection.begin();
                connection.write("OJ");    // force JSON mode
